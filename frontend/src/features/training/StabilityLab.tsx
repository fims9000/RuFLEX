...
const mapOption = (a: StudyStabilityAnalysis): EChartsOption => ({
  ...
  xAxis: { type: "value", name: "selected confidence", min: .5, max: 1 }, yAxis: { type: "value", name: "class agreement", min: .5, max: 1 },
  visualMap: { show: false, dimension: 2, pieces: [{ lte: 0, color: "#c84040" }, { gt: 0, color: "#2d7f68" }] },
  series: [{ type: "scatter", symbolSize: 9, data: a.cases.map((c) => [confidence(c.selected_run_probability), c.predicted_class_agreement, confidence(c.selected_run_probability) >= a.high_confidence_threshold && c.predicted_class_agreement < a.unstable_agreement_threshold ? 0 : 1]) }],
});
...
export function StabilityLab({ project, study, theme }: { project: ProjectSummary; study: TrainingStudy | null; theme: StudioTheme }) {
  ...
  async function createAnalysis() {
    ...
    try { const next = await studioApi.createStudyStabilityAnalysis(project.session_id, study.study_id); setAnalysis(next); setCaseId(next.cases[0]?.case_id ?? ""); }
  ...
  async function createGate() {
    if (!analysis) return;
    setBusy(true); setError(null);
    try { const evaluation = await studioApi.createAnalysisEvaluation(project.session_id, analysis.selected_run_id); setPolicy(await studioApi.createStabilityGatePolicy(project.session_id, analysis.analysis_id, evaluation.evaluation_id, .9, .8, .15)); }
  ...
  return <section className="feature-workspace">
    ...
    {analysis && <div className="run-summary-strip"><div><span>Validation cases</span><strong>{analysis.case_count}</strong></div><div><span>HC instability rate (raw)</span><strong>{analysis.high_confidence_instability_rate === null ? "N/A" : `${(analysis.high_confidence_instability_rate * 100).toFixed(1)}%`}</strong></div><div><span>High-confidence denominator</span><strong>{analysis.high_confidence_case_count}</strong></div><div><span>Unstable among them</span><strong>{analysis.high_confidence_unstable_case_count}</strong></div></div>}
    ...
    {analysis && applicable && <ChartSurface title="Case Stability Map · red = high-confidence unstable" option={mapOption(analysis)} theme={theme}/>}
    ...
    {analysis && applicable && <section className="comparison-card"><span className="eyebrow">CASE INSPECTOR</span><select aria-label="Stability case" value={selected?.case_id ?? ""} onChange={(event) => setCaseId(event.target.value)}>{analysis.cases.map((x) => <option key={x.case_id} value={x.case_id}>{x.case_id}</option>)}</select>{selected && <><p><strong>Selected run: {selected.selected_run_class} at {confidence(selected.selected_run_probability).toFixed(2)} confidence</strong> · agreement {(selected.predicted_class_agreement * 100).toFixed(0)}% · std {selected.std_probability.toFixed(3)} · support {selected.run_support_count}/{analysis.run_ids.length}</p>{decision && <p><strong>{decision.disposition}</strong> · reason: {decision.reasons.join(", ") || "all frozen criteria met"}</p>}<div className="data-table-wrap"><table className="data-table"><thead><tr><th>training run</th><th>probability</th><th>class</th></tr></thead><tbody>{Object.entries(selected.run_probabilities).map(([runId, probability]) => <tr key={runId}><td>{runId.slice(0, 12)}</td><td>{probability.toFixed(4)}</td><td>{selected.run_labels[runId]}</td></tr>)}</tbody></table></div></>}</section>}
