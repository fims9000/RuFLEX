...
export function EvidenceWorkspace({
  ...
  async function exportBundle() { setBusy(true); setError(null); try { setBundle(await studioApi.exportVerificationBundle(project.session_id)); } catch(reason) { setError(reason instanceof Error ? reason.message : String(reason)); } finally { setBusy(false); } }
  ...
  return (
    <section className="evidence-workbench">
      ...
      <section className="evidence-section"><div className="feature-toolbar compact-toolbar"><div><span className="eyebrow">ASSURANCE CASE</span><h3>Independent evidence gates and unresolved risks</h3><p>No universal trust score is produced.</p></div><Button view="action" disabled={busy || project.read_only} onClick={buildAssurance}>{busy ? "Building…" : "Build AssuranceCase"}</Button></div>{assurance && <div className="trace-card" data-testid="assurance-case"><p>{assurance.scientific_note}</p><div className="data-table-wrap"><table className="data-table"><thead><tr><th>gate</th><th>status</th><th>evidence / risk</th></tr></thead><tbody>{assurance.gates.map((gate) => <tr key={gate.key}><td>{gate.key.replaceAll("_", " ")}</td><td><StatusBadge tone={gate.status === "PASS" ? "success" : gate.status === "FAIL" ? "danger" : "warning"}>{gate.status}</StatusBadge></td><td>{gate.evidence.join(", ") || gate.risk || "—"}</td></tr>)}</tbody></table></div>{assurance.unresolved_risks.map((risk) => <p className="property-description" key={risk}>Unresolved risk · {risk}</p>)}</div>}</section>
      <section className="evidence-section"><div className="feature-toolbar compact-toolbar"><div><span className="eyebrow">VERIFICATION BUNDLE</span><h3>Inspection-first evidence export</h3><p>Exports declarative evidence and checksums; it excludes executable code, pickle/joblib, raw datasets, credentials, caches and node_modules.</p></div><Button view="outlined" disabled={busy || project.read_only || !assurance} onClick={exportBundle}>Export VerificationBundle</Button></div>{bundle && <div className="trace-card" data-testid="verification-bundle"><p>{bundle.entry_count} inspection entries · SHA-256 {bundle.sha256}</p><small>{bundle.path}</small></div>}</section>
      <section className="evidence-section">
