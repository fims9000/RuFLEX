...
export type StudyStabilityAnalysis = {
  ...
  task: "binary_classification"; run_ids: string[]; training_seeds: number[]; split_seed: number | null; evaluation_case_identity: string | null; validation_alignment_status: "EXACT_MATCH" | "MIXED_CASE_IDENTITIES" | "NOT_APPLICABLE"; applicability: "APPLICABLE" | "NOT_APPLICABLE"; applicability_reason: string | null;
  ...
  cases: Array<{ case_id: string; source_row: number | null; run_support_count: number; run_support_fraction: number; target: number; selected_run_probability: number; selected_run_class: number; mean_probability: number; std_probability: number; min_probability: number; max_probability: number; probability_range: number; predicted_class_agreement: number; positive_vote_fraction: number; vote_entropy: number; run_probabilities: Record<string, number>; run_labels: Record<string, number> }>;
...
export type StabilityGatePolicy = {
  policy_id: string; study_id: string; stability_analysis_id: string; selected_run_id: string; evaluation_id: string; calibration_id: string | null; dataset_fingerprint: string | null; dataset_artifact_sha256: string | null; model_kind: string; source_split: "validation"; fit_sample_identity: string; run_ids: string[]; required_run_support: number; probability_source: "raw"; analysis_schema_version: number;
  min_confidence: number; min_class_agreement: number; max_probability_std: number; test_status: "LOCKED_NOT_EVALUATED";
  decisions: Array<{ case_id: string; disposition: "ACCEPT" | "REVIEW" | "BLOCK"; reasons: Array<"LOW_CONFIDENCE" | "RUN_DISAGREEMENT" | "HIGH_DISPERSION" | "OUT_OF_SCOPE" | "INSUFFICIENT_RUN_SUPPORT">; selected_run_probability: number; confidence: number; class_agreement: number; probability_std: number; run_support_count: number }>;
