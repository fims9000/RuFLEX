...
def get_model_params_dict(model, task_type, dataset, num_rules, mf_class, epochs, batch_size, lr,n_patience,optim_var):
    """
...
def visualize_results(app,model):
    ...
    if app.task_var.get() == "Регрессия":
        ...
        if code == "scatter":
            ...
            ax.scatter(app.y_test, app.y_pred, s=40, alpha=0.8, color='deepskyblue', label="Пары", zorder=1)
            ax.scatter(app.y_test, app.y_test, s=40, alpha=0.3, color='limegreen', label="y = x", zorder=2)
