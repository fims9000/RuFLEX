...
class NeuroFuzzyMaster:
    def __init__(self, root):
        ...
        self.btn_load = ttk.Button(top_frame, text="Загрузить данные", command=self.load_data)
        self.btn_load.pack(side=tk.LEFT, padx=5)
        self.btn_analyze = ttk.Button(top_frame, text="Выполнить анализ", command=self.run_analysis, state=tk.DISABLED)
        self.btn_analyze.pack(side=tk.LEFT, padx=5)
        self.btn_corr = ttk.Button(top_frame, text="Корреляционная матрица", command=self.show_corr_matrix)
        self.btn_corr.pack(side=tk.LEFT, padx=5)
        self.btn_export = ttk.Button(top_frame, text="Экспорт правил", command=self.export_rules, state=tk.DISABLED)
        self.btn_export.pack(side=tk.LEFT, padx=5)
        self.btn_save_model = ttk.Button(top_frame, text="Сохранить модель", command=self.save_model, state=tk.DISABLED)
        self.btn_save_model.pack(side=tk.LEFT, padx=5)
        self.btn_load_model = ttk.Button(top_frame, text="Загрузить модель", command=self.load_model)
        self.btn_load_model.pack(side=tk.LEFT, padx=5)
        self.btn_predict = ttk.Button(top_frame, text="Анализировать с моделью", command=self.analyze_with_loaded_model, state=tk.DISABLED)
        self.btn_predict.pack(side=tk.LEFT, padx=5)
        self.progress = ttk.Progressbar(top_frame, orient=tk.HORIZONTAL, length=200, mode='determinate', style="green.Horizontal.TProgressbar")
        self.progress.pack(side=tk.RIGHT, padx=5)
        self.progress_label = ttk.Label(top_frame, text="Ожидание действия", font=("Arial", 12, "bold"))
