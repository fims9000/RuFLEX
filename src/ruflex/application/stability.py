...
def create_study_stability_analysis(project_root: Path, study_id: UUID, *, evaluation_id: UUID | None = None, threshold_id: UUID | None = None, high_confidence_threshold: float = 0.9, unstable_agreement_threshold: float = 0.8) -> StudyStabilityAnalysis:
    ...
    evaluation = None if evaluation_id is None else load_validation_evaluation(project_root, evaluation_id)
    threshold = None if threshold_id is None else load_decision_threshold(project_root, threshold_id)
    if (evaluation is None) != (threshold is None):
        raise TrainingError("Operational stability analysis requires both its validation Evaluation and DecisionThreshold policy.")
    if evaluation is not None:
        if evaluation.run_id != selected.run_id or evaluation.task != "binary_classification":
            raise TrainingError("Stability analysis Evaluation must be the selected binary TrainingRun validation evidence.")
        assert threshold is not None
        if threshold.evaluation_id != evaluation.evaluation_id or threshold.run_id != selected.run_id or threshold.source_split != "validation":
            raise TrainingError("Stability analysis DecisionThreshold provenance does not match the selected validation Evaluation.")
        if threshold.probability_source != "raw" or threshold.calibration_id is not None:
            raise TrainingError("Stability analysis currently supports only an exact raw validation DecisionThreshold policy.")
    ...
    common = dict(
        ...
        class_threshold_id=None if threshold is None else threshold.threshold_id,
        decision_threshold=None if threshold is None else threshold.selected_threshold,
    ...
    for key in sorted(next(iter(by_run.values()))):
        ...
        labels = {} if threshold is None else {run_id: int(value >= threshold.selected_threshold) for run_id, value in probabilities.items()}
        values = np.asarray(list(probabilities.values()), dtype=float)
        positive_fraction = None if threshold is None else float(np.mean(list(labels.values())))
        majority_agreement = None if positive_fraction is None else float(max(positive_fraction, 1.0 - positive_fraction))
        majority_class = None if positive_fraction is None else int(positive_fraction >= 0.5)
        entropy = None if positive_fraction is None or positive_fraction in {0.0, 1.0} else float(-(positive_fraction * math.log2(positive_fraction) + (1.0 - positive_fraction) * math.log2(1.0 - positive_fraction)))
    ...
    return _persist_analysis(project_root, StudyStabilityAnalysis(
        **common, validation_alignment_status="EXACT_MATCH", applicability="APPLICABLE", evaluation_case_identity=_identity("validation-cases", sorted(by_run[str(selected.run_id)])),
        ...
        high_confidence_instability_rate=(len(unstable) / len(high) if high else None), warnings=[] if hcir_note is None else [hcir_note],
    ))
...
def create_stability_gate_policy(project_root: Path, analysis_id: UUID, evaluation_id: UUID, *, min_confidence: float, min_class_agreement: float, max_probability_std: float, calibration_id: UUID | None = None) -> StabilityGatePolicy:
    ...
    if analysis.evaluation_id is None or analysis.class_threshold_id is None or analysis.decision_threshold is None:
        raise TrainingError("A Stability Gate requires an explicit raw validation DecisionThreshold policy; threshold-independent analysis is descriptive only.")
    ...
    threshold = load_decision_threshold(project_root, analysis.class_threshold_id)
    ...
    if evaluation.evaluation_id != analysis.evaluation_id or evaluation.run_id != analysis.selected_run_id:
        raise TrainingError("Stability Gate requires validation evidence for the selected TrainingStudy run.")
    if threshold.evaluation_id != evaluation.evaluation_id or threshold.run_id != analysis.selected_run_id or threshold.source_split != "validation" or threshold.probability_source != "raw" or threshold.calibration_id is not None or threshold.selected_threshold != analysis.decision_threshold:
        raise TrainingError("Stability Gate DecisionThreshold provenance is invalid or no longer matches its frozen analysis.")
    ...
    for case in analysis.cases:
        ...
        if case.selected_run_agreement is None or case.majority_class_agreement is None or case.selected_run_class is None:
            raise TrainingError("Operational Stability Gate requires threshold-derived labels in every matched case.")
        if case.selected_run_agreement < min_class_agreement: reasons.append("RUN_DISAGREEMENT")
        ...
        decisions.append(StabilityGateDecision(case_id=case.case_id, disposition="ACCEPT" if not reasons else "REVIEW", reasons=reasons, selected_run_probability=case.selected_run_probability, confidence=confidence, majority_class_agreement=case.majority_class_agreement, selected_run_agreement=case.selected_run_agreement, probability_std=case.std_probability, run_support_count=case.run_support_count))
...
def evaluate_frozen_stability_probabilities(policy: StabilityGatePolicy, probabilities: dict[str, float], *, out_of_scope: bool = False) -> StabilityGateApplication:
    """Apply frozen thresholds to already-computed probabilities without fitting."""
    if policy.class_threshold_id is None or policy.decision_threshold is None:
        raise TrainingError("This legacy Stability Gate has no explicit frozen DecisionThresholdPolicy provenance.")
    if set(probabilities) != {str(run_id) for run_id in policy.run_ids}:
        raise TrainingError("Stability Gate application requires probabilities from every frozen TrainingRun.")
    if str(policy.selected_run_id) not in probabilities:
        raise TrainingError("Stability Gate probabilities omit the frozen selected TrainingRun.")
    selected_probability = probabilities[str(policy.selected_run_id)]
    labels = {run_id: int(value >= policy.decision_threshold) for run_id, value in probabilities.items()}
    selected_label = labels[str(policy.selected_run_id)]
    positive_fraction = float(np.mean(list(labels.values())))
    majority_agreement = float(max(positive_fraction, 1.0 - positive_fraction))
    selected_agreement = float(np.mean([label == selected_label for label in labels.values()]))
    dispersion = float(np.std(list(probabilities.values()), ddof=0))
    confidence = max(selected_probability, 1.0 - selected_probability)
    reasons: list[str] = []
    disposition = "BLOCK" if out_of_scope else "ACCEPT"
    if out_of_scope:
        reasons.append("OUT_OF_SCOPE")
    else:
        if confidence < policy.min_confidence: reasons.append("LOW_CONFIDENCE")
        if selected_agreement < policy.min_class_agreement: reasons.append("RUN_DISAGREEMENT")
...
def apply_stability_gate_policy(project_root: Path, policy_id: UUID, sample: dict[str, float], *, metadata: dict[str, object] | None = None, generalization_contract_id: UUID | None = None) -> StabilityGateApplication:
    ...
    if policy.class_threshold_id is None or policy.decision_threshold is None:
        raise TrainingError("Stability Gate has no explicit frozen DecisionThresholdPolicy provenance.")
    threshold = load_decision_threshold(project_root, policy.class_threshold_id)
    if (
        analysis.applicability != "APPLICABLE"
        or policy.run_ids != analysis.run_ids
        or threshold.evaluation_id != policy.evaluation_id
        or threshold.run_id != policy.selected_run_id
        or threshold.source_split != "validation"
        or threshold.probability_source != "raw"
        or threshold.calibration_id is not None
        or threshold.selected_threshold != policy.decision_threshold
    ):
    ...
    if generalization_contract_id is not None:
        ...
        if scope.disposition == ScopeDisposition.BLOCK:
            out_of_scope = True
    return evaluate_frozen_stability_probabilities(policy, probabilities, out_of_scope=out_of_scope)
